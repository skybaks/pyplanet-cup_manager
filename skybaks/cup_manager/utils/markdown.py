...
def create_table(
    ...
    for header in column_header:
        ...
        column_title_justs.append(
            header["title_just"] if "title_just" in header else None
        )
    ...
    if include_header:
        ...
        for length, title, just in zip(
            column_lengths, column_titles, column_title_justs
        ):
            if just == "right":
        ...
        text.append(
            str("=" * (sum(column_lengths) + ((len(column_data) - 1) * column_spacing)))
        )
