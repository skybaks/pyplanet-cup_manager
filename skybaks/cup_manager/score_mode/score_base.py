...
class ScoreModeBase(ABC):
    ...
    def __init__(self) -> None:
        ...
        self.score_names = ScoreNames(
            score1_name="Score", score2_name="Score", scoreteam_name="Team Score"
        )
    ...
    def relevant_score_str(
        self, score: TeamPlayerScore, maniascript_format: str = None
    ) -> str:
        ...
        if maniascript_format:
            score_str = ",".join(
                [f"$<{maniascript_format}{score_item}$>" for score_item in score_items]
            )
        else:
    ...
    def diff_scores_str(self, score: TeamPlayerScore, other: TeamPlayerScore) -> str:
        ...
        if self.use_scoreteam:
            ...
            diff_items.append(
                times.format_time(teamscore_diff)
                if score.team_score_is_time
                else str(teamscore_diff)
            )
        if self.use_score2:
            score2_diff = abs(other.player_score2 - score.player_score2)
            diff_items.append(
                times.format_time(score2_diff)
                if score.player_score2_is_time
                else str(score2_diff)
            )
        if self.use_score1:
            score1_diff = abs(other.player_score - score.player_score)
            diff_items.append(
                times.format_time(score1_diff)
                if score.player_score_is_time
                else str(score1_diff)
            )
