...
class PayoutsView(OptionsView):
    ...
    def __init__(
        self, app, score_data: "list[TeamPlayerScore]", score_sorting: ScoreModeBase
    ) -> None:
    ...
    async def get_info_header_fields(self) -> "list[dict]":
        fields = [{"name": "Selected Payout:", "width": 30, "index": "name"}]
        return fields
            ...
            options.append(
                {
                    ...
                    and self.selected_option["name"] == key,
                }
            )
        frame = options
    ...
    async def get_info_data(self) -> "list[dict]":
        info_data = []
        if self.selected_option and "name" in self.selected_option:
            payout_score = await self.app.get_data_payout_score(
                self.selected_option["name"], self.score_data, self.score_sorting
...
class PresetsView(OptionsView):
    ...
    async def button_pressed(self, player, *args, **kwargs):
        if "name" in self.selected_option and self.selected_option["name"]:
            await self.app.command_setup(
                player=player,
                data=Namespace(**{"preset": self.selected_option["name"]}),
            )
            await self.close(player=player)
...
class ScoreModeView(OptionsView):
    ...
    async def get_info_header_fields(self) -> "list[dict]":
        fields = [
            {"name": "Selected:", "width": 20, "index": "name"},
            {
                "name": "",
                "width": 0,
                "index": "brief",
    ...
    async def get_option_data(self) -> "list[dict]":
        ...
        for score_mode_id, score_mode_class in self.option_data.items():
            ...
            new_option = {
                ...
                # for view
                "selected": self.selected_option
                and self.selected_option["id"] == score_mode_id,
            }
