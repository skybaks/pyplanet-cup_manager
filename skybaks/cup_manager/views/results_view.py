...
class ResultsView(ManualListView):
    ...
    def __init__(
        ...
        app: any,
        player: any,
        scores_query: "list[int]",
        scores_sorting: ScoreModeBase,
    ):
        super().__init__(self)
        self.app = app
    ...
    async def get_buttons(self) -> "list[dict[str, any]]":
        ...
        for extern_button in self.external_buttons:
            if (
                (
                    iscoroutinefunction(extern_button["visible"])
                    and await extern_button["visible"](player=self.player, view=self)
                )
                or (
                    isfunction(extern_button["visible"])
                    and extern_button["visible"](player=self.player, view=self)
                )
    ...
    async def get_data(self) -> "list[dict[str, any]]":
        ...
        scores = await self.app.results.get_data_scores(
            self.scores_query, self.scores_sorting
            ...
            items.append(
                {
                    ...
                    "player_score2_str": highlight
                    + str(player_score.player_score2_str),
